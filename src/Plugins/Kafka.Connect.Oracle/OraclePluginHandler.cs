...
public class OraclePluginHandler(
    ...
    : PluginHandler(configurationProvider)
{
    ...
    public override Task Startup(string connector) => oracleCommandHandler.Initialize(connector);
    ...
    public override IDictionary<string, Command> Commands(string connector) => oracleCommandHandler.Get(connector);

    public override JsonNode NextCommand(CommandRecord command, List<ConnectRecord> records) =>
        oracleCommandHandler.Next(command, records.Where(r => r.Status is Status.Published or Status.Skipped or Status.Triggered)
            .Select(r => r.Deserialized).ToList());
    
    public override Task Purge(string connector) => oracleCommandHandler.Purge(connector);
