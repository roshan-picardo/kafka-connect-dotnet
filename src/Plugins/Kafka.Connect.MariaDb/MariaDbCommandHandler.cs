                    ...
                    using (logger.Track($"Making sure trigger is attached to {command.Schema}.{command.Table}"))
                    {
                        var lookupTrigger = $"""
                                             ...
                                             WHERE 
                                                 ...
                                                 trigger_name = 'trg_{command.Table}_audit_log'
                                             """;
                        ...
                        var combinedTrigger = $"""
                                              DROP TRIGGER IF EXISTS {command.Schema}.trg_{command.Table}_audit_log;
                                              
                                              CREATE TRIGGER {command.Schema}.trg_{command.Table}_audit_log
                                              ...
                                              BEGIN
                                                  ...
                                                  IF (TG_OP = 'INSERT') THEN
                                                      ...
                                                      SET old_values = NULL;
                                                      SET new_values = JSON_OBJECT({string.Format(string.Join(',', columns), "NEW")});
                                                  ELSEIF (TG_OP = 'UPDATE') THEN
                                                      SET operation = 'UPDATE';
                                                      SET old_values = JSON_OBJECT({string.Format(string.Join(',', columns), "OLD")});
                                                      SET new_values = JSON_OBJECT({string.Format(string.Join(',', columns), "NEW")});
                                                  ...
                                                  VALUES
                                                      ('{command.Schema}', '{command.Table}', operation, old_values, new_values);
                                              END;
                                              """;
                        await new MySqlCommand(combinedTrigger, connection).ExecuteNonQueryAsync();
                    }
                }
            }
