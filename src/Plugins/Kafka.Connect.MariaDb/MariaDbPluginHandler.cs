...
public class MariaDbPluginHandler(
    ...
    : PluginHandler(configurationProvider)
{
    ...
    public override Task Startup(string connector) => mariaDbCommandHandler.Initialize(connector);
    ...
    public override async Task Put(IList<ConnectRecord> records, string connector, int taskId)
    {
        using (logger.Track("Putting batch of records"))
        {
            ...
            var connection = mariaDbClientProvider.GetMariaDbClient(connector, taskId).GetConnection();
    ...
    public override IDictionary<string, Command> Commands(string connector) => mariaDbCommandHandler.Get(connector);

    public override JsonNode NextCommand(CommandRecord command, List<ConnectRecord> records) =>
        mariaDbCommandHandler.Next(command, records.Where(r => r.Status is Status.Published or Status.Skipped or Status.Triggered)
            .Select(r => r.Deserialized).ToList());
    
    public override Task Purge(string connector) => mariaDbCommandHandler.Purge(connector);
