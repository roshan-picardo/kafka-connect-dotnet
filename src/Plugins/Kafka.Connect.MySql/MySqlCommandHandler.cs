...
public interface IMySqlCommandHandler
{
...
public class MySqlCommandHandler(
    ...
    ILogger<MySqlCommandHandler> logger)
    : IMySqlCommandHandler
{
    public async Task Initialize(string connector)
    {
        ...
        if (config.Changelog != null)
        {
            try
            {
                var connection = mySqlClientProvider.GetMySqlClient(connector).GetConnection();
                ...
                // Create trigger function for MySQL
