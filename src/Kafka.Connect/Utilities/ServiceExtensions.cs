...
internal static class ServiceExtensions
{
    ...
    internal static IServiceCollection AddServices(this IServiceCollection services, IConfiguration configuration)
    {
        services
            ...
            .AddScoped<ILeaderSubTask, LeaderSettingsConsumerSubTask>()
            .AddScoped<ILeaderSubTask, LeaderSettingsPublisherSubTask>()
            ...
            .AddScoped<IAsyncDeserializer<GenericRecord>>(provider =>
                new AvroDeserializer<GenericRecord>(provider.GetService<ISchemaRegistryClient>()))
            .AddScoped<IAsyncDeserializer<JsonNode>>(provider =>
                new AvroDeserializer<JsonNode>(provider.GetService<ISchemaRegistryClient>()))
            .AddScoped<IAsyncSerializer<GenericRecord>>(provider =>
                new AvroSerializer<GenericRecord>(provider.GetService<ISchemaRegistryClient>()))
            .AddScoped<IAsyncSerializer<JsonNode>>(provider =>
                new JsonSerializer<JsonNode>(provider.GetService<ISchemaRegistryClient>()))
            ...
            .Configure<LeaderConfig>(configuration.GetSection("leader"),
                options => options.BindNonPublicProperties = true)

            .AddSingleton<Providers.ConfigurationProvider>()
            .AddSingleton<Providers.IConfigurationProvider>(sp =>
                sp.GetRequiredService<Providers.ConfigurationProvider>())
            .AddSingleton<Plugin.Providers.IConfigurationProvider>(sp =>
                sp.GetRequiredService<Providers.ConfigurationProvider>())
