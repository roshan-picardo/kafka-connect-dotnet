...
{
    internal class JTokenDestructurePolicy : IDestructuringPolicy
    {
        ...
        private static LogEventPropertyValue Destructure(JObject jo, ILogEventPropertyValueFactory propertyValueFactory)
        {
            ...
            foreach (var prop in jo.Properties())
            {
                if (prop.Name == "$type")
                {
                    if (prop.Value is JValue {Value: string} typeVal)
                    {
        ...
        private static LogEventPropertyValue DestructureToDictionaryValue(JObject jo,
