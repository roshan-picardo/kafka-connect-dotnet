...
public class SqlServerPluginHandlerTests
{
    [Fact]
    public async Task Startup_DelegatesToCommandHandler()
    {
        var handler = Substitute.For<ISqlServerCommandHandler>();
    ...
    {
        var handler = Substitute.For<ISqlServerCommandHandler>();
        handler.Next(Arg.Any<CommandRecord>(), Arg.Any<IList<ConnectMessage<JsonNode>>>())
    ...
    public async Task Get_WhenReadFails_ThrowsConnectDataException()
    {
        ...
        clientProvider.GetSqlServerClient("c1", 1).Returns(client);

        var sqlExecutor = Substitute.For<ISqlServerSqlExecutor>();
        sqlExecutor.QueryRowsAsync(Arg.Any<SqlConnection>(), Arg.Any<string>())
    ...
    public async Task Put_WhenSaveAffectsZeroRows_SetsSkipped()
    {
        ...
        var sqlExecutor = Substitute.For<ISqlServerSqlExecutor>();
        sqlExecutor.ExecuteNonQueryAsync(Arg.Any<SqlConnection>(), Arg.Any<string>()).Returns(0);
    ...
    {
        var method = typeof(SqlServerPluginHandler).GetMethod("GetConnectRecord", BindingFlags.NonPublic | BindingFlags.Static);
    ...
    private static SqlServerPluginHandler NewSut(
        ...
        ISqlServerCommandHandler handler = null)
    {
        ...
        handler ??= Substitute.For<ISqlServerCommandHandler>();

        return new SqlServerPluginHandler(
            configProvider,
            factory,
            handler,
            clientProvider,
            sqlExecutor,
            Substitute.For<ILogger<SqlServerPluginHandler>>());
    }
}
