...
public class OraclePluginHandlerTests
{
    [Fact]
    public async Task Startup_DelegatesToCommandHandler()
    {
        var handler = Substitute.For<IOracleCommandHandler>();
    ...
    public async Task Get_WhenReadFails_ThrowsConnectDataException()
    {
        ...
        strategy.Build<string>("c1", Arg.Any<IConnectRecord>())
            .Returns(Task.FromResult(new StrategyModel<string> { Status = Status.Selecting, Model = "SELECT 1 FROM DUAL" }));
        ...
        sqlExecutor.QueryRowsAsync(Arg.Any<global::Oracle.ManagedDataAccess.Client.OracleConnection>(), Arg.Any<string>())
    ...
    public async Task Put_WhenSaveAffectsZeroRows_SetsSkipped()
    {
        ...
        var sqlExecutor = Substitute.For<IOracleSqlExecutor>();
        sqlExecutor.ExecuteNonQueryAsync(Arg.Any<global::Oracle.ManagedDataAccess.Client.OracleConnection>(), Arg.Any<string>())
            .Returns(0);
    ...
    {
        var method = typeof(OraclePluginHandler).GetMethod("GetConnectRecord", BindingFlags.NonPublic | BindingFlags.Static);
    ...
    private static OraclePluginHandler NewSut(
        ...
        IOracleCommandHandler handler = null)
    {
        ...
        handler ??= Substitute.For<IOracleCommandHandler>();
        return new OraclePluginHandler(
            configProvider,
            factory,
            handler,
            clientProvider,
            sqlExecutor,
            Substitute.For<ILogger<OraclePluginHandler>>());
    }
}
