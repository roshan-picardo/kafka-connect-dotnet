...
public class ReadStrategyTests
{
    ...
    public async Task BuildModels_SourceMode_ReturnsRowNumberQuery()
    {
        ...
        Assert.Contains("FETCH FIRST 100 ROWS ONLY", result.Model);
        Assert.Contains("DB2INST1.orders", result.Model);
        Assert.Contains("JSON_OBJECT(*) AS after", result.Model);
    }
    ...
    public async Task BuildModels_InitialSnapshot_ReturnsCountQuery()
    {
        ...
        Assert.Contains("CURRENT TIMESTAMP", result.Model);
        Assert.Contains("DB2INST1.orders", result.Model);
    }
    ...
    public async Task BuildModels_ChangelogMode_ReturnsAuditLogQuery()
    {
        ...
        Assert.Equal(Status.Selecting, result.Status);
        Assert.Contains("LOG_BEFORE AS before", result.Model);
        Assert.Contains("LOG_AFTER AS after", result.Model);
