...
public class Db2PluginHandlerTests
{
    [Fact]
    public async Task Startup_DelegatesToCommandHandler()
    {
        var handler = Substitute.For<IDb2CommandHandler>();
    ...
    public async Task Get_WhenReadFails_ThrowsConnectDataException()
    {
        ...
        clientProvider.GetDb2Client("c1", 1).Returns(client);
    ...
    public async Task Put_WhenSaveAffectsZeroRows_SetsSkipped()
    {
        ...
        var sqlExecutor = Substitute.For<IDb2SqlExecutor>();
        sqlExecutor.ExecuteNonQueryAsync(Arg.Any<DB2Connection>(), Arg.Any<string>()).Returns(0);
    ...
    {
        var method = typeof(Db2PluginHandler).GetMethod("GetConnectRecord", BindingFlags.NonPublic | BindingFlags.Static);
    ...
    private static Db2PluginHandler NewSut(
        ...
        IDb2CommandHandler handler = null)
    {
        ...
        handler ??= Substitute.For<IDb2CommandHandler>();

        return new Db2PluginHandler(
            configProvider,
            factory,
            handler,
            clientProvider,
            sqlExecutor,
            Substitute.For<ILogger<Db2PluginHandler>>());
    }
}
