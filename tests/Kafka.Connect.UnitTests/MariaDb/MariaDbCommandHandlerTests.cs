...
public class MariaDbCommandHandlerTests
{
    [Fact]
    public void Get_ReturnsConfiguredCommands()
    {
        ...
        var sut = new MariaDbCommandHandler(configProvider, Substitute.For<IMariaDbClientProvider>(), Substitute.For<IMariaDbSqlExecutor>(), Substitute.For<ILogger<MariaDbCommandHandler>>());
    ...
    {
        var sut = new MariaDbCommandHandler(Substitute.For<IConfigurationProvider>(), Substitute.For<IMariaDbClientProvider>(), Substitute.For<IMariaDbSqlExecutor>(), Substitute.For<ILogger<MariaDbCommandHandler>>());
    ...
    public async Task Initialize_WhenChangelogAndConnectionFails_IsHandled()
    {
        ...
        clientProvider.GetMariaDbClient("c1", -1).Returns(new FailingMariaDbClient());

        var sut = new MariaDbCommandHandler(configProvider, clientProvider, Substitute.For<IMariaDbSqlExecutor>(), Substitute.For<ILogger<MariaDbCommandHandler>>());
    ...
    [Fact]
    public void Next_ForSnapshotBatch_DisablesSnapshotWhenTotalIsGreaterOrEqualId()
    {
        var sut = new MariaDbCommandHandler(Substitute.For<IConfigurationProvider>(), Substitute.For<IMariaDbClientProvider>(), Substitute.For<IMariaDbSqlExecutor>(), Substitute.For<ILogger<MariaDbCommandHandler>>());
        var config = new CommandConfig
        {
            Snapshot = new SnapshotConfig { Enabled = true, Total = 50, Id = 3 }
        };
        var command = new CommandRecord
        {
            Command = JsonSerializer.SerializeToNode(config),
        ...
        var records = new List<ConnectMessage<JsonNode>>
        {
            new() { Value = JsonNode.Parse("{\"id\":10}") }
        };
    ...
    [Fact]
    public void Next_ForChangelogWithNoRecords_KeepsSnapshotValues()
    {
        var sut = new MariaDbCommandHandler(Substitute.For<IConfigurationProvider>(), Substitute.For<IMariaDbClientProvider>(), Substitute.For<IMariaDbSqlExecutor>(), Substitute.For<ILogger<MariaDbCommandHandler>>());
        var config = new CommandConfig
        {
            Snapshot = new SnapshotConfig { Enabled = false, Timestamp = 10, Id = 2 }
        };
        var command = new CommandRecord
        {
            Command = JsonSerializer.SerializeToNode(config),
        ...
        Assert.Equal(2, next!["Snapshot"]!["Id"]!.GetValue<long>());
        Assert.Equal(10, next["Snapshot"]!["Timestamp"]!.GetValue<long>());
    }
    ...
    public async Task Purge_WhenNoChangelog_DoesNothing()
    {
        ...
        clientProvider.DidNotReceive().GetMariaDbClient(Arg.Any<string>(), Arg.Any<int>());
    }
    ...
    public async Task Purge_WhenConnectionFails_IsSwallowed()
    {
        ...
        clientProvider.GetMariaDbClient("c1", -1).Returns(_ => throw new Exception("db down"));
        var sut = new MariaDbCommandHandler(configProvider, clientProvider, Substitute.For<IMariaDbSqlExecutor>(), Substitute.For<ILogger<MariaDbCommandHandler>>());
    ...
    [Fact]
    public async Task Purge_WhenConnectionIsNull_StillHandlesFailurePath()
    {
        var configProvider = Substitute.For<IConfigurationProvider>();
        var clientProvider = Substitute.For<IMariaDbClientProvider>();
        var client = Substitute.For<IMariaDbClient>();
        client.GetConnection().Returns((MySqlConnector.MySqlConnection)null);
    ...
    public async Task Initialize_WhenAuditObjectsMissing_CreatesTableAndTriggers()
    {
        ...
        var sql = Substitute.For<IMariaDbSqlExecutor>();
        ...
        var sut = new MariaDbCommandHandler(configProvider, clientProvider, sql, Substitute.For<ILogger<MariaDbCommandHandler>>());
    ...
    private sealed class FailingMariaDbClient : IMariaDbClient
    {
        public string ApplicationName => "failing";

        public MySqlConnector.MySqlConnection GetConnection()
        {
            var conn = new MySqlConnector.MySqlConnection("Server=127.0.0.1;Port=1;Database=db;User Id=u;Password=p;Connection Timeout=1;");
            conn.Open();
            return conn;
        }
    }
