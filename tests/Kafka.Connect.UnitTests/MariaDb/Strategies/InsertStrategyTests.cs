...
public class InsertStrategyTests
{
    ...
    public async Task Build_WithConnectRecord_ReturnsInsertSql()
    {
        ...
        configProvider.GetPluginConfig<PluginConfig>("c1").Returns(new PluginConfig { Schema = "s", Table = "t" });

        var strategy = new InsertStrategy(logger, configProvider);
        ...
        Assert.Contains("INSERT INTO `s`.`t`", result.Model);
    }
